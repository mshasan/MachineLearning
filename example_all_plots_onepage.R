...
mplot_full <- function(tag, score, splits = 8, subtitle = NA, model_name = NA, 
  ...
  return(
    grid.arrange(
      p1, p2, p3, p4,
      widths = c(1.3,1),
      layout_matrix = rbind(c(1,2), c(1,2), c(1,3), c(4,3)), labels=letters[1:4])
  ) 
}
...
df <- bmk %>% dplyr::rename("tag" = "AEFL")
results <- h2o_automl(bmk, seed = 1)
mplot_full(tag = results$scores$tag, 
           score = results$scores$norm_score,
